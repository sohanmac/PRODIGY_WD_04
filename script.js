...
async function initializeChatPage() {
  ...
  // Set default active chat to General (id = 0)
  activeContact = { id: 0, name: 'General' };
  if (chatTitle) chatTitle.textContent = activeContact.name === 'General' ? 'General Chat' : activeContact.name;
  ...
  // load general chat messages immediately
...
async function loadContacts() {
  ...
  try {
    ...
    if (contactsPanel) {
      contactsPanel.style.display = contacts.length ? '' : 'none';
    }
    if (contacts.length > 0 && (!activeContact || activeContact.id !== 0)) {
